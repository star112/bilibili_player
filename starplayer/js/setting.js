var aibk = {
	versions:function(){
		...
		iPhone: u.indexOf('iPhone') > -1 || u.indexOf('UCBrowser') > -1, //是否为iPhone或者QQHD浏览器
	...
	'play':function(url){
		...
		$(function() {
 			$(".starplayer-setting-speeds,.starplayer-setting-speed-item").on("click", function() {
    			$(".speed-stting").toggleClass("speed-stting-open");
			});
   			$(".speed-stting .starplayer-setting-speed-item").click(function() {
        			$(".starplayer-setting-speeds  .title").text($(this).text());
    		});
		});
		$(".starplayer-fulloff-icon").on("click", function() {
			aibk.dp.fullScreen.cancel();
		});
		$(".starplayer-showing").on("click", function() {aibk.dp.play();$(".vod-pic").remove();});
...
// 		});
	...
	'video':{
		...
		'con_play':function(){
			...
			$(".starplayer-cplayer").append(cplayer);
	...
	'jump':{
		'def':function(){
			h =".starplayer-setting-jfrist label";
			l =".starplayer-setting-jlast label";
			f = "#fristtime";
			j = "#jumptime";
	...
	'danmu':{
		'send':function(){
			g = $(".yzm-starplayer-send-icon");
			d = $("#dmtext");
			h = ".starplayer-comment-setting-";
		...
		'list':function(){
			$(".starplayer-list-icon,.yzm-starplayer-send-icon").on("click", function() {
				$(".list-show").empty();
    				$.ajax({
		...
		if (config.dmrule) {
		    $("div.starplayer-comment-box:last").append(liyih);
		}
		
		$(".starplayer-watching-number").text(up.usernum);
		$(".starplayer-info-panel-item-title-amount .starplayer-info-panel-item-title").html("违规词");
		for(var i=0;i<up.pbgjz.length;i++){
		var gjz_html="<e>"+up.pbgjz[i]+"</e>";
		$("#vod-title").append(gjz_html);
		}
		add('.starplayer-list-icon', ".starplayer-danmu", 'show');
	...
	'player':{
		...
		'adplay':function(url){
			...
			aibk.ad=new starplayer({
				...
				$('.starplayer-controller,.starplayer-cplayer,.starplayer-logo,#loading-box,.starplayer-controller-mask').remove();
				$('.starplayer-mask').show();
